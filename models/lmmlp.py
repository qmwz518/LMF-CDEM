...
class LMMLP(nn.Module):
    ...
    def forward(self, x, mod=None, coord=None, only_layer0=False, skip_layer0=False):
                ...
                if self.mod_up_merge and coord is not None:
                    ...
                    if self.mod_scale:
                        ...
                        if coord is not None:
                            x *= (F.grid_sample(
                                mod[:, start: end, :, :], coord.flip(-1),
                                mode='nearest', align_corners=False) + 1.0)
                        else:
                            x *= (mod[:, start: end] + 1.0)
                    ...
                    if self.mod_shift:
                        ...
                        if coord is not None:
                            x += F.grid_sample(
                                mod[:, mid_dim + start: mid_dim + end, :, :], coord.flip(-1),
                                mode='nearest', align_corners=False)
                        else:
        ...
        # print(f'x = x.view(-1, x.shape[-1]).shape={x.shape}')#qumud
        if only_layer0:
            return self.layers[0](x)
        ...
        # print(f'mod.shape{mod.shape}, mid_dim={mid_dim}') #qumud

        for idx, module in enumerate(self.layers):
            if not (skip_layer0 and idx == 0):
                # print('lmmlp117',x.shape, ' ', module)
                x = module(x)
